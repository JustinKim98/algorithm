...
int main()
{
    ...
    Number memberNum(members.size(), false);
    Number fanNum(fans.size(), false);
    ...
    for(int idx = 0; idx < fans.size(); ++idx)
    {
        if(fans[idx]=='M')
            fanNum[fans.size() - idx -1] = 1;
        else
            fanNum[fans.size() - idx -1] = 0;
    }

    auto result = karatsuba(memberNum, fanNum);

    int numNonZero = 0;
    for(int i =members.size()-1; i < result.size() - (members.size()-1); ++i)
        if(result[i] == 0)
            numNonZero += 1;
